...
-- Tabla eventos: referenciada por índices y políticas RLS del esquema, pero
-- no definida en el archivo original. Se crea aquí para que la base local
-- quede consistente desde cero.
...
-- The catalog seed predates the multi-clinic column. Associate it with a
-- deterministic local clinic so a clean local reset can run without secrets.
INSERT INTO veterinarias (id, nombre, estado)
...
ALTER TABLE catalogo
    ALTER COLUMN veterinaria_id SET DEFAULT public.local_default_veterinaria_id();
...
ALTER TABLE catalogo ALTER COLUMN veterinaria_id DROP DEFAULT;
